...
public class EntityDataBuilder {
	...
	public static <T> T buildObject(Map<String, Object> rowMap, Class<?> cls) {
		...
		rowMap.forEach((key, value) -> {
			Field colling = MetaAccessorUtil.findFieldMeta(entity.getClass(), key, ReflectionAccess.PRIVATE);
			if (colling != null) {
				if (ValidationUtil.isProjectClass(colling.getType())) {

				}
				PropertyAccessorUtil.setProperty(entity, colling.getName(), value);
			} else {
				unmapped.add(key);
			}
		});
